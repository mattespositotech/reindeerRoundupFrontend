...
interface DateFormProps extends FormProps {
    setDateForm: (value: string) => void;
}

interface MessageFormProps extends FormProps {
    setMessageForm: (value: string) => void;
}

export default function RoundupPageOld() {
    ...
    const forms = [
        ...
        <DateForm setDateForm={setDateForm} prevStep={goToPreviousStep} nextStep={goToNextStep} activeStep={activeStep} />,
        <MessageForm setMessageForm={setMessageForm} prevStep={goToPreviousStep} nextStep={goToNextStep} activeStep={activeStep} />
    ]
...
function DateForm({ setDateForm, prevStep, nextStep, activeStep }: DateFormProps) {
    const { register, handleSubmit } = useForm({ mode: 'onTouched' });

    function dateSubmit(data: FieldValues) {
        setDateForm(data.listDate)
        nextStep()
    }
    ...
    return (
        <Form>
            ...
            />
            <Button onClick={prevStep} disabled={activeStep === 1}>Previous</Button>
            <Button onClick={handleSubmit(dateSubmit)}>Next</Button>
        </Form>
    )
}

function MessageForm({ setMessageForm, prevStep, nextStep, activeStep }: MessageFormProps) {
    const { register, handleSubmit } = useForm({ mode: 'onTouched' });

    function messageSubmit(data: FieldValues) {
        setMessageForm(data.listMessage)
        nextStep()
    }
    ...
    return (
        <Form>
            ...
            />
            <Button onClick={prevStep} disabled={activeStep === 1}>Previous</Button>
            <Button onClick={handleSubmit(messageSubmit)}>Next</Button>
        </Form>
    )
}
